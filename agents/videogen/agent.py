...
async def generate_clip(
            ...
            try:
                ...
                while not operation.done:
                    ...
                    try:
                        ...
                        logger.warning(
                            "Veo poll error (attempt %d), retrying: %s",
                            attempt + 1, poll_err,
                        )
                        time.sleep(5)
                        operation = client.operations.get(operation)
                ...
                if attempt < _VEO_MAX_RETRIES:
                    wait = (attempt + 1) * 10
                    logger.warning(
                        "Veo generation failed (attempt %d/%d), retrying in %ds: %s",
                        attempt + 1, _VEO_MAX_RETRIES + 1, wait, e,
                    )
                    time.sleep(wait)
                else:
                    raise RuntimeError(
                        f"Veo generation failed after {_VEO_MAX_RETRIES + 1} attempts: {last_error}"
                    ) from last_error
    ...
    with trace_span("videogen", "generate_clip", session_id=session_id) as span:
        ...
        logger.info(
            "Veo generation completed in %.1fs (model=%s, attempt=%d)",
            metadata["latency_s"],
            _MODEL,
            metadata["attempt"],
        )
